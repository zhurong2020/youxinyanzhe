...
打开Moomoo量化功能，点击右上角"查看量化文档"，了解基本框架。
...
**进阶方法（更专业）**：
1. 使用VS Code打开策略代码
2. 安装AI插件（如Continue、GitHub Copilot）
3. 在编辑器内直接询问AI
...
**多策略的好处**：
1. **分散风险**：不同策略对应不同市场状态
2. **多元收入**：定投赚趋势、网格赚波动、Wheel赚期权费
3. **自动平衡**：市场变化时，总有策略在赚钱
4. **复利效应**：多个策略的收益可以相互增强

### 2.3 真实案例：一个账户，三重收入

```
某投资者的策略组合（$50,000账户）：
- DCA定投策略：$20,000投TQQQ（捕捉趋势）
- 网格交易策略：$20,000做NVDA网格（赚取波动）
- Wheel策略：$10,000卖TSLA期权（收取权利金）
...
**关键洞察**：单一策略可能在某些市场环境下表现不佳，但多策略组合能够适应各种市场状态，实现更稳定的收益。
