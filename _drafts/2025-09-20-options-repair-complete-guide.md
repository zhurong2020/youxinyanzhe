...
⚡ **关键优势**：这是一套**全自动化系统**，无需你判断市场走势、无需高抛低吸、无需盯盘。每天开盘时运行一次策略，系统会自动：
- 扫描最佳期权机会
- 计算最优Delta和行权价
- 执行开仓/平仓操作
- 管理止盈止损
...
与市面上其他"半自动"工具不同，我们的系统：
- **不需要你判断**："现在是不是底部？" → 系统根据数学模型决定
- **不需要你分析**："该卖多少Delta的期权？" → 系统自动计算最优值
- **不需要你纠结**："要不要止盈？" → 系统按预设规则自动执行
- **不需要你盯盘**："今天有机会吗？" → 每天运行一次，系统自动捕捉
...
智能解套模式是一套针对套牢持仓的**全自动化**期权策略系统，通过Option Scanner识别机会，Wheel Strategy执行解套，实现系统化的成本降低。

🤖 **自动化程度**：
- **无需人工判断**：不用分析K线、不用判断支撑阻力
- **无需盯盘**：每天开盘运行一次，策略自动执行
- **无需人工计算**：系统自动计算最优Delta、行权价、止盈点
- **无需情绪管理**：机械化执行，避免恐慧和贪婪
...
当`repair_mode = 1`时，Wheel策略会**完全自动**：
